package com.mff.githubapp.data.mocked
...
object MockedGithubRepository : GithubRepositoryApi {
    override suspend fun getUser(username: String): Result<User> {
        ...
        return if (user == null) Result.failure(IOException("User not found")) else Result.success(
            user
        )
    }
    ...
    override suspend fun getUserRepository(username: String): Result<List<GithubRepository>> {
        ...
        user.onSuccess {
            ...
            return if (repositories == null) Result.failure(IOException("Failed to get repositories")) else Result.success(
                repositories
            )
        ...
        return Result.failure(IOException("Failed to get repositories"))
    }
}